...
/****** Yoloworld ****/
GetResultRectYoloworld::GetResultRectYoloworld()
{
}
...
float GetResultRectYoloworld::sigmoid(float x)
{
    return 1 / (1 + fast_exp(-x));
}

int GetResultRectYoloworld::GenerateMeshgrid()
{
    int ret = 0;
    if (headNum == 0)
    {
        printf("=== Yoloworld Meshgrid  Generate failed! \n");
    }
    ...
    // printf("=== Yoloworld Meshgrid  Generate success! \n");

    return ret;
}

int GetResultRectYoloworld::GetConvDetectionResult(float **pBlob, std::vector<float> &DetectiontRects)
{
    int ret = 0;
    if (meshgrid.empty())
    {
